...
public class AConstructionManager {
	...
	public static int countNotFinishedConstructionsOfType(AUnitType type) {
		return Select.ourNotFinished().ofType(type).count() + countNotStartedConstructionsOfType(type);
	}

	/**
	 * If we requested to build building A and even assigned worker who's travelling
	 * to the building site, it's still doesn't count as unitCreated. We need to
	 * manually count number of constructions and only then, we can e.g. "count
	 * unstarted barracks constructions".
	 */
	...
	/**
	 * Issues request of constructing new building. It will automatically find
	 * position and builder unit for it.
	 */
	...
	public static void requestConstructionOf(AUnitType building, ProductionOrder order, APosition near) {
		...
		// prüft ob das Gebäude eine spezielle Behandlung bedarf und führt diese dann
		// aus (zB ein Addon)
		...
		newConstructionOrder.setMaxDistance(-1); // Für Gebäude, wird später näher definiert
	...
	/**
	 * Returns true if given worker has been assigned to construct new building or
	 * if the constructions is already in progress. Gibt true zurück, wenn der
	 * übergebene Arbeiter dem Bau eines Gebäudes zugewiesen wurde oder das Gebäude
	 * bereits in bau ist.
	 */
